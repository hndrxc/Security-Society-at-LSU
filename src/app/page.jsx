  ...
  return (
    <PageShell user={user} profile={profile} currentPath="/">
      ...
      <div className={discordServerId ? "lab-program-layout" : ""}>
        <section className="lab-programs" aria-label="Programs">{highlights.map((item, index) => <Reveal key={item.title} delay={index * 0.06}><Panel as="article" className="lab-program"><p className="lab-eyebrow"><span>0{index + 1}</span><span aria-hidden="true">↗</span></p><h3>{item.title}</h3><p>{item.detail}</p></Panel></Reveal>)}</section>
