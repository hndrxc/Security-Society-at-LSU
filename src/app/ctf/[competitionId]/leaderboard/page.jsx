...
export default async function LeaderboardPage({ params }) {
  ...
  return <PageShell user={user} profile={profile} currentPath="/ctf" wide>
    <Breadcrumbs items={[{label:'Events',href:'/events#ctf'},{label:competition.title,href:`/ctf/${competitionId}`},{label:'Leaderboard'}]} />
    <PageHeading eyebrow="CTF / Rankings" title="Leaderboard" description={competition.title}><Badge>{challengeCount || 0} challenges</Badge></PageHeading>
    <LeaderboardTable entries={leaderboard} currentUserId={user?.id} limit={100} detailed challengeCount={challengeCount} />
  </PageShell>;
}
