...
async function getVisibleEvents(supabase, now) {
  ...
  for (
    let attempt = 0;
    attempt <= EVENT_OPTIONAL_COLUMNS.length;
    attempt += 1
  ) {
    ...
    const missingColumn = EVENT_OPTIONAL_COLUMNS.find(
      (column) =>
        availableOptionalColumns.has(column) &&
        isMissingColumn(result.error, column),
    );
  ...
  return (
    <PageShell user={user} profile={profile} currentPath="/events">
      <PageHeading
        ...
        description="Find our latest meetups, workshops, and CTF competitions in one place."
      >
        <Badge>
          {events?.length || 0} Events · {competitions?.length || 0} CTFs
        </Badge>
      </PageHeading>
      <nav className="lab-section-nav" aria-label="Operations sections">
        <ActionLink href="#events" variant="secondary">
          Club Events ↓
        </ActionLink>
        <ActionLink href="#ctf" variant="secondary">
          CTF Competitions ↓
        </ActionLink>
      </nav>
      <section id="events">
        <div className="lab-section-title">
          <div>
            <p className="lab-eyebrow">Calendar / Get involved</p>
            <h2>Club Events</h2>
          </div>
          <span className="lab-eyebrow">{events?.length || 0} Scheduled</span>
        </div>
        {eventsError && <Feedback tone="error">Unable to load events</Feedback>}
        {!events?.length && !eventsError && (
          <Panel>
            <p className="lab-eyebrow">Awaiting mission briefing</p>
            <p className="lab-muted mt-3">
          ...
          {events?.map((event, index) => (
            <Reveal key={event.id} delay={index * 0.04}>
              <article className="lab-event">
                ...
                <Panel as="div" className="lab-event-body">
                  {event.image_path && supabaseUrl && (
                    <div className="relative aspect-[21/9]">
                      <Image
                        ...
                        alt=""
                        fill
                        sizes="(min-width: 1200px) 980px, (min-width: 760px) calc(100vw - 200px), calc(100vw - 120px)"
                        className="object-cover"
                        unoptimized
                      />
                    </div>
                  )}
                  <div className="lab-event-content">
                    <h3>{event.title}</h3>
                    <div className="lab-event-meta">
                      {event.location && <span>{event.location}</span>}
                      <span>
                        START:{" "}
                        {formatDate(
                          event.starts_at,
                          event.timezone || DEFAULT_TIME_ZONE,
                        )}
                      </span>
                      {event.ends_at && (
                        <span>
                    ...
                    {event.description && (
                      <p className="lab-muted text-sm">{event.description}</p>
                    )}
                    {competitionsById.has(event.ctf_competition_id) && (
                      ...
                      >
                        View CTF:{" "}
                        {competitionsById.get(event.ctf_competition_id).title} ↗
                      </ActionLink>
                    )}
                  </div>
                </Panel>
      ...
      <section id="ctf">
        <div className="lab-section-title">
          <div>
            <p className="lab-eyebrow">Competition / Put your skills to work</p>
            <h2>CTF Competitions</h2>
            <p className="lab-muted mt-3 text-sm">
              Capture flags, solve challenges, and climb the leaderboard.
            </p>
          </div>
          <span className="lab-eyebrow">
            {competitions?.length || 0} Available
          </span>
        </div>
        {!user && (
        ...
        {competitionsError && (
          <Feedback tone="error">Unable to load competitions</Feedback>
        )}
        {!competitions?.length && !competitionsError && (
          <Panel>
            <p className="lab-eyebrow">No active competitions</p>
            <p className="lab-muted mt-3">
              Check back soon for upcoming CTF events.
            </p>
          </Panel>
        )}
        <div className="lab-competition-grid">
          {competitions?.map((competition, index) => {
            const status = getCompetitionStatus(competition, now);
            return (
              <Reveal key={competition.id} delay={index * 0.06}>
                <Link
                  href={`/ctf/${competition.id}`}
                  className="lab-panel lab-competition-card"
                >
                  <div className="lab-card-top">
                    ...
                    <span className="lab-eyebrow">
                      CTF / {String(index + 1).padStart(2, "0")}
                    </span>
                  </div>
                  <TransitionRegion
                  ...
                  {competition.description && (
                    <p className="lab-muted text-sm line-clamp-2">
                      {competition.description}
                    </p>
                  )}
                  <div className="lab-muted font-terminal text-xs leading-6">
                    <p>START: {formatDate(competition.starts_at)}</p>
                    <p>END: {formatDate(competition.ends_at)}</p>
                  </div>
                  <div className="lab-card-bottom">
                    <span>
                      {challengeCounts[competition.id] || 0} challenges
                      {user
                        ? ` · ${userSolves[competition.id] || 0} solved`
                        : ""}
                    </span>
                    <span className="text-amber-300">Enter competition ↗</span>
                  </div>
                </Link>
              </Reveal>
            );
