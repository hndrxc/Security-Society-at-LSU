...
export default function ChallengeBrowser({
  ...
  const visible = (c) =>
    (category === "all" || (c.category || "misc") === category) &&
    (!isLoggedIn ||
      status[0] === "all" ||
      (status[0] === "solved"
        ? solved.has(c.id) || confirmed[c.id]
        : !solved.has(c.id) && !confirmed[c.id]));
  ...
  return (
    <section aria-label="Challenges">
        ...
        {isLoggedIn && (
          <div className="lab-filter-field">
            ...
            <ToggleGroup
              ...
              value={status}
              onValueChange={(value) => {
                if (value.length) setStatus(value);
              }}
            >
              {["all", "unsolved", "solved"].map((value) => (
      ...
      <p role="status" className="lab-muted text-xs mb-5">
        {count} of {challenges.length} challenges
        {count === 0 ? " · No challenges match these filters." : ""}
      </p>
      {challenges.map((challenge) => (
        <Activity
          key={challenge.id}
          mode={visible(challenge) ? "visible" : "hidden"}
        >
          <ChallengeCard
            challenge={challenge}
            isSolved={
            ...
            onSolve={(result) => {
              setConfirmed((prev) => ({ ...prev, [challenge.id]: result }));
              router.refresh();
            }}
          />
        </Activity>
